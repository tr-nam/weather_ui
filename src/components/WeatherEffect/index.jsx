...
const WeatherEffect = ({
  ...
  const [loadError, setLoadError] = useState(null);
  const modelCache = useRef({}); // Cache for loaded GLTF models
  ...
  const getModelFallback = useCallback(
    (type) => {
      ...
      switch (type) {
        case 'sun':
          return () => {
            const geometry = new THREE.SphereGeometry(1.5, 16, 16); // Giảm phân đoạn
            const material = new THREE.MeshBasicMaterial({
              color: currentTimeOfDay === 'night' ? 0xccccff : 0xffff99,
              transparent: true,
              opacity: 0.9,
        ...
        case 'cloud':
          return () => {
            const geometry = new THREE.SphereGeometry(0.8, 8, 8); // Giảm phân đoạn
            const material = new THREE.MeshStandardMaterial({
              color: currentTimeOfDay === 'night' ? 0x333344 : 0xdddddd,
              transparent: true,
              opacity: 0.9,
        ...
        case 'mountain':
          return () => {
            const geometry = new THREE.ConeGeometry(7, 10, 4); // Giảm số mặt
            const material = new THREE.MeshStandardMaterial({
              color: 0x555566,
              roughness: 0.9,
              flatShading: true,
  ...
  useEffect(() => {
    ...
    renderer.setSize(mountRef.current.clientWidth, mountRef.current.clientHeight);
    renderer.setPixelRatio(Math.min(window.devicePixelRatio, 1.5)); // Giảm pixel ratio
    renderer.outputColorSpace = THREE.SRGBColorSpace;
    renderer.toneMapping = THREE.ACESFilmicToneMapping;
    renderer.toneMappingExposure = 1.2;
    mountRef.current.appendChild(renderer.domElement);
    ...
    const bloomPass = new UnrealBloomPass(
      new THREE.Vector2(window.innerWidth / 2, window.innerHeight / 2), // Giảm độ phân giải
      0.3, // Giảm cường độ bloom
      0.4,
      0.85
    );
    composer.addPass(bloomPass);
    ...
    const setBackgroundColor = () => {
      ...
      const context = canvas.getContext('2d');
      const gradient = context.createLinearGradient(0, 128, 0, 0);
      gradient.addColorStop(0, '#67aeeb');
    ...
    const loadMountain = () => {
      if (mountainLoadedRef.current) return;
      if (modelCache.current.mountain) {
        const mountain = modelCache.current.mountain.clone();
      ...
      loader.load(
        ...
        (gltf) => {
          ...
          scene.add(mountain);
          modelsRef.current.mountain = mountain;
          modelCache.current.mountain = mountain.clone();
          mountainLoadedRef.current = true;
        },
        undefined,
        () => {
    ...
    const animate = () => {
      if (!isVisibleRef.current || !sceneRef.current || !cameraRef.current) {
        animationFrameRef.current = requestAnimationFrame(animate);
        return;
      }
      const now = Date.now();
      const isIdle = now - lastInteractionRef.current > 10000; // 10s không tương tác
      if (isIdle && Math.random() > 0.5) { // Giảm tần suất cập nhật khi idle
        animationFrameRef.current = requestAnimationFrame(animate);
        return;
      }
      updateParticles();
    ...
    const handleResize = () => {
      ...
      if (width === rendererRef.current.domElement.width && height === rendererRef.current.domElement.height) return;
    ...
    return () => {
      ...
      particlesRef.current = {};
      Object.values(modelsRef.current).forEach((model) => {
        if (Array.isArray(model)) model.forEach((item) => sceneRef.current.remove(item));
        else if (model) sceneRef.current.remove(model);
      });
      modelsRef.current = { clouds: [] };
      sceneRef.current.traverse((object) => {
        if (object.geometry) object.geometry.dispose();
  ...
  const updateSunPosition = useCallback(() => {
    ...
    sun.position.x += Math.sin(Date.now() * 0.0001) * 5;
    sun.position.y = 20 + Math.sin(Date.now() * 0.0002) * 2;
    sun.visible = true;
  }, []);
  ...
  useEffect(() => {
    ...
    particlesRef.current = { lastUpdate: 0 };
    modelsRef.current.clouds.forEach((cloud) => scene.remove(cloud));
    if (modelsRef.current.sun) {
      scene.remove(modelsRef.current.sun);
      modelsRef.current.sun = null;
    }
    ...
    scene.fog = null; // Không cần sương mù cho sunny

    const dirLight = scene.children.find((child) => child instanceof THREE.DirectionalLight);
    if (dirLight) {
      dirLight.color.setHex(0xffffcc);
    ...
    // Tải mặt trời
    const loadSun = () => {
      if (modelCache.current.sun) {
        const sun = modelCache.current.sun.clone();
        sun.scale.set(0.3, 0.3, 0.3);
        const sunLight = new THREE.PointLight(0xffffcc, 1, 100);
        sun.add(sunLight);
        scene.add(sun);
      ...
      loader.load(
        ...
        (gltf) => {
          ...
          scene.add(sun);
          modelsRef.current.sun = sun;
          modelCache.current.sun = sun.clone();
        },
        undefined,
        () => {
          const sun = getModelFallback('sun')();
    ...
    // Tải mây (ít mây cho sunny)
    const loadClouds = () => {
      const cloudCount = 3; // Giảm số lượng mây
      const geometry = new THREE.BufferGeometry();
      const material = new THREE.MeshStandardMaterial({
        color: 0xdddddd,
        transparent: true,
      ...
      for (let i = 0; i < cloudCount; i++) {
        ...
        const scale = 12 + Math.random() * 12;
        matrix.setPosition(position);
        matrix.scale(new THREE.Vector3(scale, scale, scale));
        cloudMesh.setMatrixAt(i, matrix);
        modelsRef.current.clouds.push({ position, scale });
      }
      scene.add(cloudMesh);
      modelsRef.current.cloudMesh = cloudMesh;
    };
    ...
    return () => {};
  }, [weatherCondition, precipitationProbability, getTimeOfDay, getModelFallback]);

  const updateParticles = useCallback(() => {
    const now = performance.now();
    if (particlesRef.current.lastUpdate && now - particlesRef.current.lastUpdate < 32) return; // Giảm tần suất cập nhật
    particlesRef.current.lastUpdate = now;
  }, []);

  const updateModels = useCallback(() => {
    if (modelsRef.current.cloudMesh && modelsRef.current.clouds) {
      const matrix = new THREE.Matrix4();
      modelsRef.current.clouds.forEach((cloud, i) => {
        cloud.position.x -= 0.02;
        if (cloud.position.x < -100) {
          cloud.position.x = 100;
        ...
        cloud.position.y += Math.sin(Date.now() * 0.0005) * 0.005;
        matrix.setPosition(cloud.position);
        matrix.scale(new THREE.Vector3(cloud.scale, cloud.scale, cloud.scale));
        modelsRef.current.cloudMesh.setMatrixAt(i, matrix);
      });
      modelsRef.current.cloudMesh.instanceMatrix.needsUpdate = true;
    }
  }, []);
